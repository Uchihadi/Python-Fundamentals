...
def find_passengers_destination(destination):
    ...
    for i in ticket_list:
        lst = i.split(":")
        if (lst[2].startswith(destination)):
            count1 += 1
    return count1   
...
def find_passengers_per_flight():
    ...
    for i in ticket_list:
        string_list = i.split(":")
        if string_list[0] in flight_list:
            index = flight_list.index(string_list[0])
            no_of_pass[index] += 1
        else:
    ...
    result_list = []
    for i, j in zip (flight_list, no_of_pass):
...
def sort_passenger_list():
    ...
    dict1 = {} #Initialise the first dictionary to extricate key value pair
